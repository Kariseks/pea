...
//<time in minutes><algo id><file_path>
Parser::Parser()
...
bool Parser::parse(int argc, char* argv[]) {
    ...
    if (argc != arg_num+1) {
        errorMsg<< "Blad: Nieprawidlowa liczba argumentow.\n"
    ...
    try {
        ...
        if (maxTime <= 0) {
            errorMsg << "Blad: Maksymalny czas musi byc wartoscia dodatnia.\n";
            valid = false;
            return false;
        }
    } catch (...) {
        errorMsg << "Blad: Argument 1 (max czas) musi byc liczba calkowita.\n";
        valid = false;
        return false;
    }
    ...
    try {
        ...
        if (algorithmId < 1 || algorithmId > 10) {
            errorMsg << "Blad: Argument 2 (id) musi byc w przedziale od 1 do 10.\n";
            valid = false;
            return false;
        }
    } catch (...) {
        errorMsg << "Blad: Argument 2 (id) musi byc liczba calkowita.\n";
        valid = false;
        return false;
    }
    ...
    filePath = argv[3];
    if (filePath.empty()) {
        errorMsg << "Blad: Sciezka do pliku nie moze byc pusta.\n";
        valid = false;
        return false;
    }
    valid = true;
