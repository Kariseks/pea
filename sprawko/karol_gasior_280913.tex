...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{rys/brute_force_czas_vs_n.pdf}
	\caption{Czas obliczeń algorytmu Brute-Force w funkcji rozmiaru instancji $N$.}
	\label{fig:bf_czas}
\end{figure}

Jak można zauważyć na powyższym wykresie, czas wykonania przeglądu zupełnego rośnie w sposób drastyczny. Potwierdza to teoretyczną złożoność $O(N!)$. Dla małych wartości $N$ (np. od 5 do 10) czas jest ułamkiem sekundy, jednak po przekroczeniu pewnego progu krzywa staje się niemal pionowa, co czyni ten algorytm nieużytecznym dla większych problemów.
...
Czasy wykonania obu heurystyk rosną wielomianowo, co pozwala na rozwiązywanie instancji o wiele większych niż w przypadku algorytmu dokładnego. Widać wyraźnie, że RNN, iterujący po każdym wierzchołku startowym, jest wolniejszy od klasycznego NN, jednak dla badanych wielkości $N \le 14$ czasy obu metod nadal pozostają na poziomie ułamków sekund. Szczegółowe wartości liczbowe dla algorytmów deterministycznych zebrano w Tabeli \ref{tab:czasy}.
...
%\begin{figure}[H]
%	\centering
%	\includegraphics[width=0.8\textwidth]{rys/dokladnosc_czas_avg_nn_rnn_z_rand.pdf}
...
Ze względu na stochastyczny charakter algorytmu Random Search, zbadano również rozrzut jego dokładności podczas pojedynczych pomiarów (Rysunek \ref{fig:rand_raw} oraz Rysunek \ref{fig:rand_box}).

%\begin{figure}[H]
%	\centering
%	\includegraphics[width=0.8\textwidth]{rys/dokladnosc_czas_raw_rand.pdf}
...
%\begin{figure}[H]
%	\centering
%	\includegraphics[width=0.8\textwidth]{rys/stabilnosc_rand.pdf}
%	\caption{Wykres pudełkowy (Boxplot) ilustrujący stabilność wyników algorytmu RAND dla poszczególnych $N$.}
%	\label{fig:rand_box}
%\end{figure}

Z powyższych wizualizacji wynika, że rozrzut wyników metody RAND jest ogromny. Dla najmniejszych instancji (np. $N=5$) losowe tasowanie często trafia w optimum (100\% dokładności). Jednak wraz ze wzrostem rozpiętości instancji, szansa na wylosowanie dobrej ścieżki maleje, a mediana dokładności (czerwona linia na wykresie pudełkowym) drastycznie spada.

\subsection{Szczegółowa analiza agregacyjna algorytmu RAND}

Aby dokładniej prześledzić wpływ czasu na ewolucję rozwiązania stochastycznego, dokonano próbkowania wyników algorytmu RAND na dwóch płaszczyznach czasowych: w skali mikro (pierwsze ułamki sekund) oraz makro (od kilku sekund do kilku minut).

W Tabeli \ref{tab:rand_micro} zaprezentowano zachowanie algorytmu w przedziale od 10 do 50 mikrosekund. Z kolei Tabela \ref{tab:rand_macro} ukazuje średnią dokładność przy długotrwałym działaniu (agregacja co 5 sekund, a następnie co minutę).
...
%	}
%	\caption{Dokładność algorytmu RAND [\%] w początkowej fazie działania (przedziały rzędu mikrosekund).}
%	\label{tab:rand_micro}
%\end{table}
%
%%\begin{table}[H]
%%	\centering
...
%%	}
%%	\caption{Dokładność algorytmu RAND [\%] w długich przedziałach czasowych (od sekund do minut).}
%%	\label{tab:rand_macro}
%%\end{table}

Dane te potwierdzają, że nadanie algorytmowi losowemu gigantycznego budżetu czasowego pozwala mu stopniowo odnajdywać coraz lepsze rozwiązania, jednak proces ten jest wysoce nieefektywny i nie gwarantuje zbieżności do globalnego optimum dla dużych $N$.
